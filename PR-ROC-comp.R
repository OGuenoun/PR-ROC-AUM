...
ROC_AUM <- function(pred_tensor, label_tensor){
  is_positive = label_tensor$flatten() == 2
...
PR_AUM <- function(pred_tensor, label_tensor) {
  ...
  thresh_tensor = -pred_tensor$flatten()
  sorted_indices = torch::torch_argsort(thresh_tensor)
  sorted_fp_cum = fp_diff[sorted_indices]$cumsum(dim=1)
  sorted_fn_cum = -fn_diff[sorted_indices]$flip(dims=1)$cumsum(dim=1)$flip(dims=1)
  sorted_thresh = thresh_tensor[sorted_indices]
  sorted_is_diff = sorted_thresh$diff() != 0
  ...
  uniq_fp_after = sorted_fp_cum[sorted_fp_end]
  uniq_fn_before = sorted_fn_cum[sorted_fn_end]
  FP = torch::torch_cat(list(torch::torch_tensor(0.0), uniq_fp_after))
  FN = torch::torch_cat(list(uniq_fn_before, torch::torch_tensor(0.0)))
...
nn_ROC_AUM_loss <- torch::nn_module(
  "nn_ROC_AUM_loss",
...
nn_PR_AUM_loss <- torch::nn_module(
  "nn_PR_AUM_loss",
  inherit = torch::nn_mse_loss,
  initialize = function() {
    super$initialize()
...
n.pixels <- 28
n.epochs <- 400
measure_list <- mlr3::msrs(c("classif.auc", "classif.prauc"))
mlr3torch_loss_list <- list(
  ROC_AUM=nn_ROC_AUM_loss,
  PR_AUM=nn_PR_AUM_loss,
